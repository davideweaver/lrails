    ...
    class << self
      def handle_with_controller(exception, controller=nil, request=nil)
        if Config.should_send_to_api?
		  ...
		  Loggr::Events.create().text("handle_with_controller").data(data.uniqueness_hash).post()
        end
      end
      ...
      def handle_with_rack(exception, environment, request) 
        if Config.should_send_to_api?
		  ...
		  Loggr::Events.create().text("handle_with_rack").data(data.uniqueness_hash).post()
        end
      end
      ...
      def handle(exception, name=nil)
        if Config.should_send_to_api?
		  ...
		  Loggr::Events.create().text("handle").data(data.uniqueness_hash).post()
        end
      end
    end
  end

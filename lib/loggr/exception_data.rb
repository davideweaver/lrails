...
module Loggr
  class ExceptionData 
    ...
    def to_hash
      hash = ::Loggr::ApplicationEnvironment.to_hash(framework)
      hash.merge!({
        'exception' => {
    ...
    def context_stuff
      context = Thread.current[:loggr_context]
    ...
    def to_json
      begin
        ...
        begin
          ...
          raise StandardError.new("You need a json gem/library installed to send errors to Loggr (Object.to_json not defined). \nInstall json_pure, yajl-ruby, json-jruby, or the c-based json gem")
        end
      end
    end
